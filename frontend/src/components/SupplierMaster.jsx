...
const baseVoiceSchema = {
    supplier_code: "Extract supplier code.",
    supplier_name: "Extract supplier name.",
    supplier_full_name: "Extract full name.",
    supplier_type: "Extract supplier type.",
    sort_by: "Sort field.",
...
export default function SupplierMaster() {
    ...
    const handleSubmit = async () => {
        try {
            ...
            let res;
            if (formData.id) {
                res = await fetch(`${API_BASE}/suppliers/${formData.id}`, {
                    method: 'PUT',
            ...
            if (res.ok) {
                alert(formData.id ? "Supplier updated!" : "Supplier saved!");
                setFormData({
                    supplier_code: '',
                    supplier_name: '',
                    supplier_full_name: '',
    ...
    return (
        <div className="container" style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto' }}>
            <h2 className="page-title" style={{ fontSize: '1.5rem', fontWeight: '700', color: '#1e293b', marginBottom: '2rem' }}>Supplier Master</h2>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
                    <div>
                        <div className="form-row" style={{ marginBottom: '1rem' }}>
                            <label className="form-label" style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '500' }}>Supplier Code <span style={{ color: 'var(--danger-red)' }}>*</span></label>
                            <input type="text" className="form-control" name="supplier_code" value={formData.supplier_code} onChange={handleChange} style={{ width: '100%', padding: '0.5rem', border: '1px solid #cbd5e1', borderRadius: '6px' }} />
                        </div>
                        <div className="form-row" style={{ marginBottom: '1rem' }}>
                            <label className="form-label" style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '500' }}>Supplier Name <span style={{ color: 'var(--danger-red)' }}>*</span></label>
                            <input type="text" className="form-control" name="supplier_name" value={formData.supplier_name} onChange={handleChange} style={{ width: '100%', padding: '0.5rem', border: '1px solid #cbd5e1', borderRadius: '6px' }} />
                        </div>
                    </div>
                    <div>
                        <div className="form-row" style={{ marginBottom: '1rem' }}>
                            <label className="form-label" style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '500' }}>Full Name</label>
                            <input type="text" className="form-control" name="supplier_full_name" value={formData.supplier_full_name} onChange={handleChange} style={{ width: '100%', padding: '0.5rem', border: '1px solid #cbd5e1', borderRadius: '6px' }} />
                        </div>
                        <div className="form-row" style={{ marginBottom: '1rem' }}>
                            <label className="form-label" style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '500' }}>Type</label>
                            <input type="text" className="form-control" name="supplier_type" value={formData.supplier_type} onChange={handleChange} style={{ width: '100%', padding: '0.5rem', border: '1px solid #cbd5e1', borderRadius: '6px' }} />
                        </div>
                        <div className="form-row" style={{ marginBottom: '1rem' }}>
            ...
            {/* LIST SECTION */}
            <div>
                <h3 className="section-header" style={{ fontSize: '1.25rem', fontWeight: '600', marginBottom: '1rem' }}>Supplier List</h3>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
                        <thead>
                            <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                                ...
                                <th style={thStyle} onClick={() => handleSort('supplier_code')}>Code {sortBy === 'supplier_code' && (sortOrder === 'asc' ? '↑' : '↓')}</th>
                                <th style={thStyle} onClick={() => handleSort('supplier_name')}>Name {sortBy === 'supplier_name' && (sortOrder === 'asc' ? '↑' : '↓')}</th>
                                <th style={thStyle} onClick={() => handleSort('supplier_full_name')}>Full Name {sortBy === 'supplier_full_name' && (sortOrder === 'asc' ? '↑' : '↓')}</th>
                                <th style={thStyle} onClick={() => handleSort('supplier_type')}>Type {sortBy === 'supplier_type' && (sortOrder === 'asc' ? '↑' : '↓')}</th>
                        ...
                        <tbody>
                            {listData.map(item => (
                                <tr key={item.id} style={{ borderBottom: '1px solid #e2e8f0', background: 'white' }}>
                                    ...
                                    <td style={{ padding: '0.75rem' }}>{item.supplier_code}</td>
                                    <td style={{ padding: '0.75rem', fontWeight: '500' }}>{item.supplier_name}</td>
                                    <td style={{ padding: '0.75rem' }}>{item.supplier_full_name}</td>
                                    <td style={{ padding: '0.75rem' }}>{item.supplier_type}</td>
                            ...
                            {listData.length === 0 && (
                                <tr>
                                    <td colSpan="6" style={{ padding: '2rem', textAlign: 'center', color: '#64748b' }}>No suppliers found</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
