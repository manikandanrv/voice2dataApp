...
const baseVoiceSchema = {
    twine_twist_code: "Extract code.",
    twine_twist_name: "Extract name.",
    twine_twist_desc: "Extract description.",
    sort_by: "Sort field.",
    sort_order: "Sort order.",
    page_size: "Page size."
};

export default function TwineTwistMaster() {
    ...
    const fetchList = async () => {
        try {
            ...
            const res = await fetch(`${API_BASE}/twine-twists/?${queryParams.toString()}`);
    ...
    const handleSubmit = async () => {
        try {
            ...
            let res;
            if (formData.id) {
                res = await fetch(`${API_BASE}/twine-twists/${formData.id}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload)
                });
            } else {
                res = await fetch(`${API_BASE}/twine-twists`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload)
                });
    ...
    return (
        <div className="container" style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto' }}>
            <h2 className="page-title" style={{ fontSize: '1.5rem', fontWeight: '700', color: '#1e293b', marginBottom: '2rem' }}>Twine Twist Master</h2>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
                    <div>
                        <div className="form-row" style={{ marginBottom: '1rem' }}>
                            <label className="form-label" style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '500' }}>Twist Code</label>
                            <input type="text" className="form-control" name="twine_twist_code" value={formData.twine_twist_code} onChange={handleChange} style={{ width: '100%', padding: '0.5rem', border: '1px solid #cbd5e1', borderRadius: '6px' }} />
                        </div>
                        <div className="form-row" style={{ marginBottom: '1rem' }}>
                            <label className="form-label" style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '500' }}>Twist Name</label>
                            <input type="text" className="form-control" name="twine_twist_name" value={formData.twine_twist_name} onChange={handleChange} style={{ width: '100%', padding: '0.5rem', border: '1px solid #cbd5e1', borderRadius: '6px' }} />
                        </div>
                    </div>
                    <div>
                        <div className="form-row" style={{ marginBottom: '1rem' }}>
                            <label className="form-label" style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '500' }}>Description</label>
                            <input type="text" className="form-control" name="twine_twist_desc" value={formData.twine_twist_desc} onChange={handleChange} style={{ width: '100%', padding: '0.5rem', border: '1px solid #cbd5e1', borderRadius: '6px' }} />
                        </div>
                        <div className="form-row" style={{ marginBottom: '1rem' }}>
            ...
            {/* LIST SECTION */}
            <div>
                <h3 className="section-header" style={{ fontSize: '1.25rem', fontWeight: '600', marginBottom: '1rem' }}>Twist List</h3>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
                        <thead>
                            <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                                ...
                                <th style={thStyle} onClick={() => handleSort('twine_twist_code')}>Code {sortBy === 'twine_twist_code' && (sortOrder === 'asc' ? '↑' : '↓')}</th>
                                <th style={thStyle} onClick={() => handleSort('twine_twist_name')}>Name {sortBy === 'twine_twist_name' && (sortOrder === 'asc' ? '↑' : '↓')}</th>
                                <th style={thStyle} onClick={() => handleSort('twine_twist_desc')}>Desc {sortBy === 'twine_twist_desc' && (sortOrder === 'asc' ? '↑' : '↓')}</th>
                        ...
                        <tbody>
                            {listData.map(item => (
                                <tr key={item.id} style={{ borderBottom: '1px solid #e2e8f0', background: 'white' }}>
                                    ...
                                    <td style={{ padding: '0.75rem' }}>{item.twine_twist_code}</td>
                                    <td style={{ padding: '0.75rem', fontWeight: '500' }}>{item.twine_twist_name}</td>
                                    <td style={{ padding: '0.75rem' }}>{item.twine_twist_desc}</td>
