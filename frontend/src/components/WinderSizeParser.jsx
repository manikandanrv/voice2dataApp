...
export default function WinderSizeParser() {
    ...
    const handleAddToMaster = async () => {
        ...
        try {
            const payload = {
                winder_yarn_size: sizeCode,
                display_name: editableDisplayName || result.generated_display_name || sizeCode,
                winder_yarn_size_description: editableDescription || result.generated_description,
                active: true
            };
            ...
            if (res.ok) {
                setAddMessage('Success: Added to Winder Master!');
    ...
    return (
        <div style={{ padding: '2rem', maxWidth: '1000px', margin: '0 auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '2rem' }}>
                ...
                <div style={cardStyle}>
                    ...
                    <form onSubmit={handleParse} style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem' }}>
                        <input
                            ...
                            placeholder="Enter Winder Code (e.g. 2W3POGM1S)"
                            style={inputStyle}
                        />
                            ...
                            {(!result.errors || result.errors.length === 0) && (
                                <>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
                                        ...
                                        <ResultItem label="Winder Bobbin" value={result.winder_bobbin} />
                                    ...
                                    <div style={{ borderTop: '1px solid #e2e8f0', paddingTop: '1rem' }}>
                                        ...
                                        >
                                            {addingMaster ? 'Adding to Master...' : '✓ Add to Winder Master'}
                                        </button>
                                        {addMessage && (
                ...
                {/* Master List Section */}
                <div style={cardStyle}>
                    <h3 style={sectionHeaderStyle}>Existing Winder Masters</h3>
                        ...
                        <tbody>
                            {masters.map(m => (
                                <tr key={m.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                    <td style={{ padding: '0.75rem', fontWeight: '600' }}>{m.winder_yarn_size}</td>
                                    <td style={{ padding: '0.75rem' }}>{m.code}</td>
                                    <td style={{ padding: '0.75rem' }}>{m.size}</td>
                                    <td style={{ padding: '0.75rem' }}>{m.display_name}</td>
                                    <td style={{ padding: '0.75rem' }}>{m.winder_yarn_size_description}</td>
