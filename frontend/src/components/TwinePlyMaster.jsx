...
const baseVoiceSchema = {
    twine_ply_code: "Extract code.",
    twine_ply_name: "Extract name.",
    twine_ply_desc: "Extract description.",
    sort_by: "Sort field.",
    sort_order: "Sort order.",
    page_size: "Page size."
};

export default function TwinePlyMaster() {
    ...
    const fetchList = async () => {
        try {
            ...
            const res = await fetch(`${API_BASE}/twine-plies/?${queryParams.toString()}`);
    ...
    const handleSubmit = async () => {
        try {
            ...
            let res;
            if (formData.id) {
                res = await fetch(`${API_BASE}/twine-plies/${formData.id}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload)
                });
            } else {
                res = await fetch(`${API_BASE}/twine-plies`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload)
                });
    ...
    return (
        <div className="container" style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto' }}>
            <h2 className="page-title" style={{ fontSize: '1.5rem', fontWeight: '700', color: '#1e293b', marginBottom: '2rem' }}>Twine Ply Master</h2>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
                    <div>
                        <div className="form-row" style={{ marginBottom: '1rem' }}>
                            ...
                            <input type="text" className="form-control" name="twine_ply_code" value={formData.twine_ply_code} onChange={handleChange} style={{ width: '100%', padding: '0.5rem', border: '1px solid #cbd5e1', borderRadius: '6px' }} />
                        </div>
                        <div className="form-row" style={{ marginBottom: '1rem' }}>
                            <label className="form-label" style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '500' }}>Ply Name</label>
                            <input type="text" className="form-control" name="twine_ply_name" value={formData.twine_ply_name} onChange={handleChange} style={{ width: '100%', padding: '0.5rem', border: '1px solid #cbd5e1', borderRadius: '6px' }} />
                        </div>
                    </div>
                    <div>
                        <div className="form-row" style={{ marginBottom: '1rem' }}>
                            <label className="form-label" style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '500' }}>Description</label>
                            <input type="text" className="form-control" name="twine_ply_desc" value={formData.twine_ply_desc} onChange={handleChange} style={{ width: '100%', padding: '0.5rem', border: '1px solid #cbd5e1', borderRadius: '6px' }} />
                        </div>
                        <div className="form-row" style={{ marginBottom: '1rem' }}>
            ...
            {/* LIST SECTION */}
            <div>
                <h3 className="section-header" style={{ fontSize: '1.25rem', fontWeight: '600', marginBottom: '1rem' }}>Ply List</h3>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
                        <thead>
                            <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                                ...
                                <th style={thStyle} onClick={() => handleSort('twine_ply_code')}>Code {sortBy === 'twine_ply_code' && (sortOrder === 'asc' ? '↑' : '↓')}</th>
                                <th style={thStyle} onClick={() => handleSort('twine_ply_name')}>Name {sortBy === 'twine_ply_name' && (sortOrder === 'asc' ? '↑' : '↓')}</th>
                                <th style={thStyle} onClick={() => handleSort('twine_ply_desc')}>Desc {sortBy === 'twine_ply_desc' && (sortOrder === 'asc' ? '↑' : '↓')}</th>
                        ...
                        <tbody>
                            {listData.map(item => (
                                <tr key={item.id} style={{ borderBottom: '1px solid #e2e8f0', background: 'white' }}>
                                    ...
                                    <td style={{ padding: '0.75rem', fontWeight: '500' }}>{item.twine_ply_name}</td>
                                    <td style={{ padding: '0.75rem' }}>{item.twine_ply_desc}</td>
