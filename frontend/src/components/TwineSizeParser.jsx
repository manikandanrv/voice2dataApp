...
const API_BASE = `${import.meta.env.VITE_API_URL}/api/twine`;

export default function TwineSizeParser() {
    ...
    const fetchMasters = async () => {
        try {
            let url = `${API_BASE}/masters?page=${masterPage}&limit=5`;
    ...
    const handleParse = async (e) => {
        ...
        try {
            const response = await fetch(`${API_BASE}/parse-size?size_string=${encodeURIComponent(sizeCode)}`, {
                method: 'POST',
    ...
    const handleAddToMaster = async () => {
        ...
        try {
            const payload = {
                ...
                twine_size_description: editableDescription || result.generated_description,
                active: true
            };
            ...
            if (res.ok) {
                setAddMessage('Success: Added to Master!');
    ...
    return (
        <div style={{ padding: '2rem', maxWidth: '1000px', margin: '0 auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '2rem' }}>
                ...
                <div style={cardStyle}>
                    ...
                    <form onSubmit={handleParse} style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem' }}>
                        <input
                            ...
                            placeholder="Enter Twine Code (e.g. 6HNM1...)"
                            style={inputStyle}
                        />
                                ...
                                <>
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
                                        <div style={{ padding: '1rem', backgroundColor: '#f0fdf4', border: '1px solid #dcfce7', borderRadius: '8px' }}>
                                            <label style={{ display: 'block', fontSize: '0.85rem', color: '#166534', fontWeight: '600', marginBottom: '0.5rem' }}>DISPLAY SIZE</label>
                                            <input
                                                type="text"
                                        ...
                                        <div style={{ padding: '1rem', backgroundColor: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: '8px' }}>
                                            <label style={{ display: 'block', fontSize: '0.85rem', color: '#0369a1', fontWeight: '600', marginBottom: '0.5rem' }}>SIZE DESCRIPTION</label>
                                            <input
                                                type="text"
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
                                        <ResultItem label="Ply" value={result.ply_name} code={result.ply_code} />
                                        ...
                                        {result.twine_color_name && <ResultItem label="Twine Color" value={result.twine_color_name} code={result.twine_color_code} />}
                                    ...
                                    <div style={{ borderTop: '1px solid #e2e8f0', paddingTop: '1rem' }}>
                                        ...
                                        >
                                            {addingMaster ? 'Adding to Master...' : '✓ Add to Twine Master'}
                                        </button>
                                        {addMessage && (
                ...
                {/* Master List Section */}
                <div style={cardStyle}>
                    <h3 style={sectionHeaderStyle}>Existing Twine Masters</h3>
                        ...
                        <tbody>
                            {masters.map(m => (
                                <tr key={m.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                    <td style={{ padding: '0.75rem', fontWeight: '600' }}>{m.twine_size}</td>
                                    <td style={{ padding: '0.75rem' }}>{m.display_name}</td>
                                    <td style={{ padding: '0.75rem' }}>{m.twine_size_description}</td>
