...
const ReelingProduction = () => {
    ...
    const fetchEntries = async () => {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE}/production/reeling/?${queryParams}`);
    ...
    const handleVoiceData = (data) => {
        ...
        if (data.weight) mappedData.actual_weight = data.weight;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (activeTab === 'production') {
                url = editingId
                    ? `${API_BASE}/production/reeling/${editingId}`
                    : `${API_BASE}/production/reeling/`;
                method = editingId ? 'PUT' : 'POST';
                payload = {
                    ...formData,
                    dhank_no: parseFloat(formData.dhank_no || 0),
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Delete this entry?')) {
            await fetch(`${API_BASE}/production/reeling/${id}`, { method: 'DELETE' });
            fetchEntries();
        }
    };
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">Reeling Production</h1>
                    ...
                    <table className="w-full">
                        <thead className="bg-gray-50/50">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Dhank</th>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {loading ? <tr><td colSpan="6" className="text-center py-4">Loading...</td></tr> : entries.map(entry => (
                                <tr key={entry.id} className="hover:bg-gray-50/50">
                                    ...
                                    <td className="px-6 py-4 text-sm text-gray-600">{entry.dhank_no}</td>
                                    ...
                                    {activeTab === 'production' && (
                                        <>
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                ...
                                                <div>
                                                    <label className="block text-sm font-medium text-gray-700 mb-1">Dhank No</label>
                                                    <input type="number" name="dhank_no" value={formData.dhank_no} onChange={handleChange} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg" />
                                                </div>
                                            </div>
...
export default ReelingProduction;
