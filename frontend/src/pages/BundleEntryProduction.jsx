...
const BundleEntryProduction = () => {
    ...
    const [masterData, setMasterData] = useState({
        machines: [],
        counts: [],
        operators: [], // Assume we can fetch operators here too
        shifts: []
    });
    ...
    useEffect(() => {
        const fetchMasters = async () => {
            try {
                // We might need to handle 404s if some endpoints don't exist, but assuming consistency
                const [machinesRes, countsRes, countsRes2, shiftsRes] = await Promise.all([
                    fetch(`${API_BASE}/master/machines/`),
                    fetch(`${API_BASE}/master/counts/`), // Assuming counts exists
                    fetch(`${API_BASE}/master/operators/`), // Verify if this exists or just use text input if not
                ...
                let operatorsData = { items: [] };
                if (countsRes2.ok) operatorsData = await countsRes2.json(); // Re-using var name slightly confusing in thought but code:
                // Actually let's use separate vars

                let shiftsData = { items: [] };
                if (shiftsRes.ok) shiftsData = await shiftsRes.json();
    ...
    const fetchEntries = async () => {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE}/production/bundle-entry/?${queryParams}`);
    ...
    useEffect(() => {
        if (activeTab === 'production') {
            const total = (parseFloat(formData.no_of_bundles) || 0) * (parseFloat(formData.bundle_weight) || 0);
            if (total !== parseFloat(formData.total_weight || 0)) {
                setFormData(prev => ({ ...prev, total_weight: total > 0 ? parseFloat(total.toFixed(2)) : '' }));
            }
        }
    }, [formData.no_of_bundles, formData.bundle_weight, activeTab]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (activeTab === 'production') {
                url = editingId
                    ? `${API_BASE}/production/bundle-entry/${editingId}`
                    : `${API_BASE}/production/bundle-entry/`;
                method = editingId ? 'PUT' : 'POST';
                payload = {
                    ...formData,
                    bundle_weight: parseFloat(formData.bundle_weight || 0),
                    no_of_bundles: parseInt(formData.no_of_bundles || 0),
                ...
                switch (activeTab) {
                    ...
                    case 'machine_stop':
                        ...
                        payload = {
                            ...
                            actual_weight: parseFloat(formData.total_weight || 0), // Use total weight here maybe?
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Delete this entry?')) {
            await fetch(`${API_BASE}/production/bundle-entry/${id}`, { method: 'DELETE' });
            fetchEntries();
        }
    };
    ...
    const voiceSchema = {
        ...
        lot_no: "Lot Number",
        bundle_weight: "Weight of one bundle",
        no_of_bundles: "Number of bundles",
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">Bundle Entry Production</h1>
                    ...
                    <table className="w-full">
                        <thead className="bg-gray-50/50">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Lot/Count</th>
                                <th className="px-6 py-3 text-right text-xs font-semibold text-gray-500 uppercase">Bundles</th>
                            ...
                            {loading ? <tr><td colSpan="6" className="text-center py-4">Loading...</td></tr> : entries.map(entry => (
                                <tr key={entry.id} className="hover:bg-gray-50/50">
                                    ...
                                    <td className="px-6 py-4 text-sm text-gray-600">
                                        <div className="font-mono">{entry.lot_no}</div>
                                        <span className="text-xs text-gray-400">{entry.count_code}</span>
                                    </td>
                                    <td className="px-6 py-4 text-sm text-right font-medium">{entry.no_of_bundles} <span className="text-xs text-gray-400">({entry.bundle_weight} each)</span></td>
                                ...
                                <form onSubmit={handleSubmit} className="space-y-4">
                                    ...
                                    {activeTab === 'production' && (
                                        <>
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <div>
                                                    <label className="block text-sm font-medium text-gray-700 mb-1">Bundle Wt <span className="text-red-500">*</span></label>
                                                    <input type="number" step="0.01" name="bundle_weight" value={formData.bundle_weight} onChange={handleChange} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg" />
                                                </div>
                                                <div>
                                                    <label className="block text-sm font-medium text-gray-700 mb-1">No Bundles</label>
                                                    <input type="number" name="no_of_bundles" value={formData.no_of_bundles} onChange={handleChange} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg" />
                                                </div>
                                            </div>
...
export default BundleEntryProduction;
