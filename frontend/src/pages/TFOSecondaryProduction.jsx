...
const TFOSecondaryProduction = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        current_doff_no: '',
        side: 'A',
        tfo_ply_batch_no: '', // Specific to Secondary
        start_time: '',
        end_time: '',
        no_of_spindles: '',
        no_of_bobbins: '',
    ...
    const fetchEntries = async () => {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE}/production/tfo-secondary/?${queryParams}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (activeTab === 'production') {
                url = editingId
                    ? `${API_BASE}/production/tfo-secondary/${editingId}`
                    : `${API_BASE}/production/tfo-secondary/`;
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Delete this entry?')) {
            await fetch(`${API_BASE}/production/tfo-secondary/${id}`, { method: 'DELETE' });
            fetchEntries();
        }
    };
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">TFO Secondary Production</h1>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {loading ? <tr><td colSpan="6" className="text-center py-4">Loading...</td></tr> : entries.map(entry => (
                                <tr key={entry.id} className="hover:bg-gray-50/50">
                                    ...
                                    <td className="px-6 py-4 text-sm text-gray-600">{entry.lot_no} <br /><span className="text-xs text-gray-400">PB: {entry.tfo_ply_batch_no}</span></td>
                                    ...
                                    {activeTab === 'production' && (
                                        <>
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                ...
                                                <div>
                                                    <label className="block text-sm font-medium text-gray-700 mb-1">Ply Batch No</label>
                                                    <input type="text" name="tfo_ply_batch_no" value={formData.tfo_ply_batch_no} onChange={handleChange} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg" />
                                                </div>
                                            </div>
...
export default TFOSecondaryProduction;
