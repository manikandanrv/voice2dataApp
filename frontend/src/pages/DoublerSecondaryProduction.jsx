...
const DoublerSecondaryProduction = () => {
    // Main Data State
    ...
    const [formData, setFormData] = useState({
        ...
        operator_name: '',
        current_doff_no: '',
        ply_doff_no: '', // Specific to Secondary
        start_time: '',
        end_time: '',
        no_of_spindles: '',
        no_of_bobbins: '',
    ...
    const fetchEntries = async () => {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE}/production/doubler-secondary/?${queryParams}`);
    ...
    const handleVoiceData = (data) => {
        ...
        if (data.weight) mappedData.bobbin_weight = data.weight;
        if (data.bobbins) mappedData.no_of_bobbins = data.bobbins;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (activeTab === 'production') {
                url = editingId
                    ? `${API_BASE}/production/doubler-secondary/${editingId}`
                    : `${API_BASE}/production/doubler-secondary/`;
                method = editingId ? 'PUT' : 'POST';
                payload = {
                    ...formData,
                    size: parseFloat(formData.size || 0),
                    standard_weight: parseFloat(formData.standard_weight || 0),
                    bobbin_weight: parseFloat(formData.bobbin_weight || 0),
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Delete this entry?')) {
            await fetch(`${API_BASE}/production/doubler-secondary/${id}`, { method: 'DELETE' });
            fetchEntries();
        }
    };
    ...
    const voiceSchema = {
        ...
        lot_no: "Lot Number",
        ply_doff_no: "Ply Doff Number",
        bobbin_weight: "Bobbin Weight",
        waste: "Waste",
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">Doubler Secondary Production</h1>
                    ...
                    <table className="w-full">
                        <thead className="bg-gray-50/50">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Lot No</th>
                                <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Ply Doff</th>
                                <th className="px-6 py-3 text-right text-xs font-semibold text-gray-500 uppercase">Bobbins</th>
                                <th className="px-6 py-3 text-right text-xs font-semibold text-gray-500 uppercase">Bobbin Wt</th>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {loading ? <tr><td colSpan="6" className="text-center py-4">Loading...</td></tr> : entries.map(entry => (
                                <tr key={entry.id} className="hover:bg-gray-50/50">
                                    ...
                                    <td className="px-6 py-4 text-sm text-gray-600 font-mono">{entry.lot_no}</td>
                                    <td className="px-6 py-4 text-sm text-gray-600">{entry.ply_doff_no}</td>
                                    <td className="px-6 py-4 text-sm text-right">{entry.no_of_bobbins}</td>
                                    <td className="px-6 py-4 text-sm text-right font-medium">{entry.bobbin_weight}</td>
                                    ...
                                    {activeTab === 'production' && (
                                        <>
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                ...
                                                <div>
                                                    <label className="block text-sm font-medium text-gray-700 mb-1">Ply Doff No</label>
                                                    <input type="text" name="ply_doff_no" value={formData.ply_doff_no} onChange={handleChange} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg" />
                                                </div>
                                            </div>

                                            <div className="grid grid-cols-2 gap-4">
                                                <div>
                                                    <label className="block text-sm font-medium text-gray-700 mb-1">No of Bobbins</label>
                                                    <input type="number" name="no_of_bobbins" value={formData.no_of_bobbins} onChange={handleChange} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg" />
                                                </div>
                                                <div>
                                                    <label className="block text-sm font-medium text-gray-700 mb-1">Bobbin Weight <span className="text-red-500">*</span></label>
                                                    <input type="number" step="0.01" name="bobbin_weight" value={formData.bobbin_weight} onChange={handleChange} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg" />
                                                </div>
                                                <div>
                                    ...
                                    {activeTab === 'machine_stop' && (
                                        <>
                                            <div className="grid grid-cols-2 gap-4">
                                                ...
                                                <div>
                                                    <label className="block text-sm font-medium text-gray-700 mb-1">Bobbin Wt (Kg)</label>
                                                    <input type="number" step="0.01" name="bobbin_weight" value={formData.bobbin_weight} onChange={handleChange} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg" />
                                                </div>
                                            </div>
                                            <div>
...
export default DoublerSecondaryProduction;
